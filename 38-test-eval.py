...
def prepareDataTest():
    ...
    for k in range(0, len(dir_nir)):  # range(0,2000):
        # normalistation
        x = np.zeros((224, 224, 4))
    ...
    return List_img_test, List_img_test_names, len(dir_nir)
...
# hist1 = model.fit(X, Y, batch_size=4, epochs=1, verbose=1, callbacks=[
...
Xtest, test_names, length = prepareDataTest()

Xtest = np.array(Xtest)
y_pred = np.array(model.predict(Xtest))
print(y_pred)
y_predi = np.max(y_pred, axis=3)
# y_predi = np.argmax(y_pred, axis=3)


for i in range(0, length):

    #y = cv2.resize(y_predi[i], (384, 384))
    # print(y_predi[i].min())
    c = np.where(y_predi[i] <= 0.5, 0, y_predi[i])

    cv2.imwrite(
        "38-Cloud-A-Cloud-Segmentation-Dataset-master/preds_folder_root/preds_folder/" + test_names[i][5:], c)
